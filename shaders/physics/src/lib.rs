...
pub fn main(
    ...
    #[spirv(workgroup)] positions_workgroup: &mut [Vec2; 1024],
    #[spirv(workgroup)] velocities_workgroup: &mut [Vec2; 1024],
) {
    // let current_cell = (global_invocation.x + PREFIX_SUM_OFFSET_HACK) as usize;
    let first_cell_in_workgroup = workgroup_invocation.x * 64;
    let last_cell_in_workgroup = first_cell_in_workgroup + 64 - 1;

    // let mut current_cell = 111;
    let current_cell =
        (first_cell_in_workgroup + local_invocation.x + PREFIX_SUM_OFFSET_HACK) as usize;

    let mut first_particle_index: usize = 0;
    // if local_invocation.x == 0 {
